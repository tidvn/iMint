...
import { createMetadata, uploadImageFromUrl } from "../../function";
...
export function MintScreen({
  ...
  const getMetadata = async () => {
    ...
    try {
      const uri = await uploadImageFromUrl(imageUrl);

      if (fields.length === 0) fields.push({ "trait_type": metadata.name, "value": metadata.name });
      ...
      return await createMetadata(body)
    } catch (error) {
